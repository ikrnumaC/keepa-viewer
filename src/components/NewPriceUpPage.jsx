...
const PriceChange = ({ change }) => {
  ...
  const numChange = Number(change);
  if (numChange === 0) {
    return <span className="ml-2 text-blue-500">→0</span>;
  }
  
  const className = numChange > 0 ? 'text-blue-500' : 'text-red-500';
  return <span className={`ml-2 ${className}`}>{numChange > 0 ? '+' : ''}{numChange}</span>;
}
...
const NewPriceUpPage = () => {
  ...
  const fetchProducts = async (useLastKey = null) => {
    try {
      setIsLoading(true);
      const url = new URL('https://yh546hgz2b.execute-api.ap-southeast-2.amazonaws.com/prod/products/new-price-up');
            ...
            <div key={product.asin} className="border rounded p-4 shadow-sm hover:shadow-md transition-shadow">
              <div className="grid grid-cols-5 gap-4">
                ...
                <div className="flex items-center justify-center">
                  <a href={`https://www.amazon.co.jp/dp/${product.asin}`} target="_blank" rel="noopener noreferrer">
                    <img 
                ...
                <div className="col-span-3">
                  ...
                  <div className="grid grid-cols-3 gap-4 mt-2">
                    ...
                    <div>
                      ...
                      <p>
                        出品数: {formatStock(product.new_count)}
                    ...
                    <div>
                      ...
                      <p>
                        {formatPrice(product.used_price)}
                        <PriceChange change={product.used_price_change} />
                      </p>
                      <p>
                        出品数: {formatStock(product.used_count)}
